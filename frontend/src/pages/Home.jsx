...
import CoinSlider from "../components/CoinSlider"; // <-- keep CoinSlider separate
...
export default function HomePage() {
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950 md:flex md:pt-20 pt-11"
      style={{ ["--color-secondary"]: "#00E676" }} // ensure your CSS var is set for components that use it
    >
      ...
      <main className="flex-1 mx-auto max-w-5xl px-4 md:px-8 py-6">
        ...
        <div className="top-4 z-40">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-secondary  flex items-center justify-center text-white text-lg font-bold">FM</div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <button aria-label="Give feedback" onClick={() => setActiveModal("feedback")} className="px-3 py-2 rounded-xl bg-secondary/80 text-black text-sm">Feedback</button>
             <Link to={'/market'} ><button className="px-3 py-2 rounded-xl bg-black text-white dark:bg-white dark:text-black text-sm">Market</button></Link> 
            </div>
          </motion.div> 
        </div>
        ...
        <motion.section initial="hidden" animate="show" variants={container} className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={fadeUp} whileHover={float.hover} className="lg:col-span-2 p-6 rounded-2xl bg-gradient-to-r dark:from-secondary from-secondary to-secondary dark:to-primary text-gray-700 dark:text-white  shadow-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }} className="mt-6">
          <CoinSlider />
        </motion.div>
        ...
        <motion.section initial="hidden" animate="show" variants={container} className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { img: coupon, title: "Coupon", note: "1 Coupon", color: 'text-gray-500'},
            { img: suspect, title: "Identity Verification", note: "Pending", color: 'text-yellow-500' },
            { img: levelUp, title: "My Level", note: "Lv. 1" , color: 'text-gray-500' },
            { img: referral, title: "Referral Bonus", note: "Invite friends" , color: 'text-gray-500' },
            { img: flag, title: "Rewards Hub", note: "Redeem" , color: 'text-gray-500' },
            { img: fileIcon, title: "MT4/MT5 Trading", note: "Connect", color: 'text-gray-500' },
          ].map((it, idx) => (
