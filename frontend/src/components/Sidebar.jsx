...
export default function SidebarAnimated() {
  ...
  const navItemClass = (active) =>
    `flex items-center gap-4 px-3 py-2 rounded-lg transition-colors duration-150 ${
      active ? "bg-gray-100 dark:bg-black/20 text-[color:var(--color-secondary)]" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-black/10"
    }`;

  return (
    <aside className="hidden md:flex flex-col justify-start dark:bg-[color:var(--color-primary)] bg-white text-black dark:text-white py-6 px-4 pt-17 w-64 sticky top-5 h-screen overflow-hidden">
      ...
      <motion.div
        ...
        className="pointer-events-none absolute right-4 top-6 w-32 h-32 rounded-full bg-gradient-to-tr from-[color:var(--color-secondary)] to-purple-500 blur-3xl"
      />

      <motion.nav variants={container} initial="hidden" animate="show" className="flex flex-col gap-3 mt-6 text-sm font-medium z-10">
        <motion.div variants={item} whileHover="hover">
          <Link to="/" className={navItemClass(currentPath === "/")}> 
            ...
            <span>Home</span>
            <motion.span className="ml-auto text-xs text-gray-400" layout>{currentPath === "/" ? "active" : ""}</motion.span>
          </Link>
        </motion.div>
          ...
          <AnimatePresence initial={false}>
            {openTrade && (
              ...
              >
                ...
                <motion.div whileHover={{ x: 6 }}>
                  <Link to="/trade/commodities" className={navItemClass(currentPath === "/trade/commodities")}>
        ...
        <motion.div variants={item} whileHover="hover">
          <div className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg transition-colors ${openTools ? "bg-gray-100 dark:bg-black/20 text-[color:var(--color-secondary)]" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-black/10"}`}>
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 grid place-items-center rounded-md bg-white/90 dark:bg-black/20 shadow-sm">
          ...
          <AnimatePresence initial={false}>
            {openTools && (
              ...
              >
                <motion.div whileHover={{ x: 6 }}>
                  <Link to="/copy-trading" className={navItemClass(currentPath === "/copy-trading")}>
                ...
                <motion.div whileHover={{ x: 6 }}>
                  <Link to="/tools/bots" className={navItemClass(currentPath === "/bots")}>
                ...
                <motion.div whileHover={{ x: 6 }}>
                  <Link to="/tools/signals" className={navItemClass(currentPath === "/signals")}>
                    <Activity className="w-4 h-4" />
        ...
        {/* Chat & Account */}
        <motion.div variants={item} whileHover="hover">
          <Link to="/message" className={navItemClass(currentPath === "/message")}>
      ...
      <div className="mt-auto pt-6 z-10">
        ...
        <Link to="/support" className="text-sm font-medium hover:underline">Contact support</Link>
      </div>
    </aside>
  );
}